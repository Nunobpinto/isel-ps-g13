...
export default class extends React.Component {
  ...
  voteUp () {
    ...
    const uri = 'http://localhost:8080/classes/' + id + '/courses/stage/' + this.state.stagedId + '/vote'
    const body = {
      method: 'POST',
      headers: {
  ...
  rejectStaged () {
    const id = this.props.classId
    const uri = 'http://localhost:8080/classes/' + id + '/courses/stage/' + this.state.stagedId
    const body = {
      method: 'DELETE',
      headers: {
        'Access-Control-Allow-Origin': '*',
  ...
  componentDidMount () {
    const id = this.props.classId
    const uri = `http://localhost:8080/classes/${id}/courses/stage`
    const header = {
      headers: {
        'Access-Control-Allow-Origin': '*',
        'Authorization': 'Basic ' + cookies.get('auth'),
