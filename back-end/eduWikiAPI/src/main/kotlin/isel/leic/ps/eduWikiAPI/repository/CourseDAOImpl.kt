...
class CourseDAOImpl : CourseDAO {
    ...
    override fun getTermsOfCourse(courseId: Int) =
            handle.createQuery(
                    "select T.${TermDAOJdbi.TERM_ID}, T.${TermDAOJdbi.TERM_SHORT_NAME}, " +
                            "T.${TermDAOJdbi.TERM_YEAR}, T.${TermDAOJdbi.TERM_TYPE}, T.${TermDAOJdbi.TERM_TIMESTAMP} " +
                            "from ${TermDAOJdbi.TERM_TABLE} as T " +
                            "inner join $COURSE_TERM_TABLE as C on T.${TermDAOJdbi.TERM_ID} = C.${COURSE_TERM_TERM_ID} " +
    ...
    override fun getSpecificTermOfCourse(courseId: Int, termId: Int) =
            handle.createQuery(
                    "select T.${TermDAOJdbi.TERM_ID}, T.${TermDAOJdbi.TERM_SHORT_NAME}, " +
                            "T.${TermDAOJdbi.TERM_YEAR}, T.${TermDAOJdbi.TERM_TYPE} " +
                            "from ${TermDAOJdbi.TERM_TABLE} as T " +
                            "inner join $COURSE_TERM_TABLE as C on T.${TermDAOJdbi.TERM_ID} = C.${TermDAOJdbi.TERM_ID} " +
                            "where C.$COURSE_ID = :courseId"
            )
                    .bind("courseId", courseId)
                    .mapTo(Term::class.java)
